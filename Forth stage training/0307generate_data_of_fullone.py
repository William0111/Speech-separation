...
# Generate input data for a specific audio(11290)
def generate_spectrogram_data():
    spec = np.zeros(500*512)
    x1 = read_Flac(11290)[0]
    x1 = x1[:16000 * 5]

    x1_spec = abs(stft(x1, win, step)).reshape(622 * 512)
    high_boundary = 622 * 512
    low_boundary = 122*512
...
def generate_mask_data():
    mask = np.zeros(500*512)
    x1 = read_Flac(11290)[0]
    x1 = x1[:16000 * 5]

    x1_spec = abs(stft(x1, win, step))
    x1_mask = get_mask(x1_spec).reshape(622 * 512)
    high_boundary = 622 * 512
    low_boundary = 122 * 512
...
x1 = read_Flac(11290)[0]
x_s = x1[:16000 * 5]
x_s = abs(stft(x_s, win, step))
x_s = get_mask(x_s)


plt.imshow(abs(x_s[:, : int(fftLen / 2 + 1)].T), aspect = "auto", cmap=plt.cm.afmhot, origin = "lower")
plt.title("Mask", fontsize = 20)
plt.show()
# =============================================================================
# 
# spectrogram_s = abs(stft(x_s, win, step)) .reshape(497* 512)
# 
# spectrogram_s = spectrogram_s[0:497*500] 
# 
# spectrogram_s =spectrogram_s.reshape(497,500).T
# 
# s1 = get_mask(spectrogram_s)
# 
# plt.imshow(abs(s1[:, : int(fftLen / 2 + 1)].T), aspect = "auto", cmap=plt.cm.afmhot, origin = "lower")
# plt.title("Mask", fontsize = 20)
# plt.show()
# 
# 
...
clear_spec_500 = generate_spectrogram_data()
clear_mask_500 = generate_mask_data()

clear_spec_500 = clear_spec_500.T

reshaped_mask = clear_mask_500.reshape(500,512)

plt.imshow(abs(reshaped_mask[:, : int(fftLen / 2 + 1)].T), aspect = "auto", cmap=plt.cm.afmhot, origin = "lower")
plt.title("Mask", fontsize = 20)
plt.show()

clear_spec_500_reshapeto1024 = clear_spec_500.reshape(250,1024)
clear_mask_500_reshapeto1024 = clear_mask_500.reshape(250,1024)

#np.savetxt('clear_spec_input_250.txt', clear_spec_500_reshapeto1024, delimiter = ' ')
#np.savetxt('clear_mask_generated_from_threshold_250.txt', clear_mask_500_reshapeto1024, delimiter = ' ')

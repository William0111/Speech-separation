...
def generate_spectrogram_data(start_from=10001, number=1000, noise_power=0, length=3):
    spec = np.zeros(5120)
    j = 0
    while j < number:
...
def generate_mask_data(start_from=10001, number=1000, noise_power=0, length=3):
    mask = np.zeros(512)
    j = 0
    while j < number:
        x1 = read_Flac(start_from + j)[0]
...
clear_spec_train_1000 = generate_spectrogram_data(start_from=10001, number=28000, noise_power=0, length=10)
clear_mask_train_1000 = generate_mask_data(start_from=10001, number=28000, noise_power=0, length=1)

np.savetxt('0307_spec_train_280000data_10columns.txt', clear_spec_train_1000, delimiter = ' ')
np.savetxt('0307_mask_train_280000data_1columns.txt', clear_mask_train_1000, delimiter = ' ')

#try to plot to check
x1 = read_Flac(12244)[0]
x1 = x1[:16000 * 2]
...
y_mask = x1_spec.reshape(247,512)
